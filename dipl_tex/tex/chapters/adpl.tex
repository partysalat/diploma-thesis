...
Um eine vektorwertige Funktion $F:\mathcal D \subset \R^n\to \R^m$ mittels Automatischem Differenzieren abzuleiten, muss sie bereits in einer algebraischen Form vorliegen, sie soll also aus Verknüpfung der Relationen $+, -, \cdot, /$ oder polynomiale arithmetische Operationen sin, cos, sqrt, exp, log usw. bestehen.
Wenn ein Computerprogramm eine Funktion ausführt, werden diese Verknüfungen stückweise nacheinander ausgewertet.
Der Compiler führt eine sogenannte \textit{Three-Part Evaluation Procedure} durch, d.h. er speichert die unabhängigen Eingangsvariablen $x\in \R^n$ in internen Variablen $(v_{1-n},\ldots,v_{-1})$ ab, führt die Berechnungsschritte $v_1,\ldots, v_{l-m}$ aus und gibt die 
