...
\]
Für alle $\phi_i$ wird angenommen, dass sie differenzierbar in einer für uns hinreichenden Umgebung sind. Dann nutzen wir die Tangentiale Linearisierung
\begin{align}
...
\begin{figure}[H]
 ...
 \begin{tabular}{|l|l|}
 \hline
 $v_{-1} =  x_1$ 		&$\Delta v_{-1} = \Delta x_1$\\
 $v_{0} =  x_2$ 		&$\Delta v_{0} = \Delta x_2$	\\
 \hline
 $v_{1} = v_{-1}\cdot v_{0}$ 	&$\Delta v_{1} = \Delta v_{0}v_1 + v_0 \Delta v_{1}$\\
 $v_{2} =  \abs(v_{1})$		&$\Delta v_{2} = |v_1 + \Delta v_1| - |v_1|$\\
 $v_{3} =  v_2+v_{-1}$ 		&$\Delta v_{3} = \Delta v_2 + \Delta v_0$\\
 $v_{4} =  \sin(v_{3})$ 	&$\Delta v_{4} = \cos(v_3) \Delta v_3$\\
 \hline
 $y =  v_{4}$ & $\Delta y = \Delta v_4$	\\
 \hline
\end{tabular}
