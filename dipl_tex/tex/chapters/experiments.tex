...
\begin{cases}
 h_t + (hu)_x = 0,\\
 (hu)_t + \left[\frac{(hu)^2}{h} + \frac{g}{2}w^2\right]_x = 0
\end{cases} 
\label{eq:swe}
\end{equation}
...
Diese kann durch Bodengegebenheiten oder Reibung erweitert werden.
...
Kurganov bietet in \cite{kurganov2007second} ein robustes Finite Volumen Schema für die SWE an, die sowohl \textit{well balanced} ist als auch den \textit{dry state} behandelt. Es wurde genutzt, um die gewünschte Ortsdiskretisierung durchzuführen.
...
\subsection{FVM Schema von Kurganov und Unglattheiten}
