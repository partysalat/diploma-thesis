...
\subsection{Gradient und Optimierung}
Bei der Konvergenz des Gradienten ist für $\theta=1$ kein besseres Verhalten der GIMP gegenüber der IMP feststellbar. 
\begin{figure}
\footnotesize
\begin{minipage}[b]{0.49\linewidth}
% \begin{minipage}[t][3cm][t]{5cm}
...
\caption*{(b) Summiert}
\end{minipage}
\caption{SWE Fehler über Zeit mit $L=50,\Delta x=10$, $x_0=U_0^{(1)},n=50$, $x_{\text{Obs}} = U_0^{(2)}, k=0.5$ und $\theta=1$}
...
\caption*{(b) Summiert}
\end{minipage}
\caption{SWE Fehler über Zeit mit $L=50,\Delta x=10$, $x_0=U_0^{(1)},n=50$, $x_{\text{Obs}} = U_0^{(2)}, k=0.5$ und $\theta=2$}
