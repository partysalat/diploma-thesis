...
\caption*{(b) $hu$}
\end{minipage}
\caption{SWE Benutzte Anfangswerte}
\label{fig:sweInitialValues}
\end{figure}

Auch bei der Shallow Water Equation erhalten wir eine Konvergenz von $\mathcal O(h^2)$. Wie im Bild \ref{fig:sweConvergence} ersichtlich, ist der Konvergenzgraph sehr stabil gegenüber der implizitem Mittelpunktsregel, jedoch erst ab zirka $10^2$ Schritten. Bei mehr als $10^3$ Schritten ist der Abstand der des Konvergenzverhaltens der impliziten Mittelpunktsregel zur GIMP nur noch marginal. 


\begin{figure}
\centering
\input{img/swe_convergence.tikz}
\caption{Konvergenz SWE Romberg im Intervall $I = [0,50], L=50, dx=10$}
...
\caption{Konvergenz SWE Romberg im Intervall $I = [0,50], L=100, dx=10$}
\label{fig:sweConvergenceRomberg}
\end{figure}
Dass die verallgemeinerte Mittelpunktsregel eine bessere Integration liefert ist auch im Bild \ref{fig:sweErrorOverTime} ersichtlich, in dem der Fehler über die Zeit geplottet wurde. Während die GIMP anfangs schlechter integriert, besitzt sie, über einen längeren Zeitraum betrachtet, einen deutlich geringeren Fehler.
...
\caption*{(b) Summiert}
\end{minipage}
\caption{SWE Fehler über Zeit mit $L=200,dx=10,h = 1$}
