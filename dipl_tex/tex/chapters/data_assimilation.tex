...
Die 4-D variationelle Datenassimilation ist eine Methode, die im Zusammenhang mit hyperbolischen partiellen Differentialgleichungen steht, z.B. der Wellengleichung.  Eingeführt von Talagrand und Le Dimet in \cite{dimet1986variational} finden die Methoden der optimalen Steuerung immer mehr Verwendung in Dateneassimilierungsmethoden für reale Beispiele(z.B. \cite{korotaev2008retrieving,elbern20004d,broquet2009application}). Benutzt wird dieses Verfahren meist in atmosphärischen Modellen, um optimale Parameter zur numerischen Wettervorhersage zu finden. Auch in ozeanographischen Modellen findet diese Methode Anwendung.
...
\end{aligned}
\end{equation}
wobei $\nabla_x H(t)$ der Gradient von $H(x,t)$ ausgewertet an der Stelle $(x(t),t)$ und $\gatdiff x$ die Gâteaux-Ableitung von $x$, also eine Störung, beschreibt. Diese ist definiert als die Lösung von
\begin{equation}
\label{eq:tlm}
\begin{aligned}
