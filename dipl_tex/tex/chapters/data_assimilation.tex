...
Die 4-D Variations-Datenassimilation ist eine Methode, welche die Theorie der optimalen Steuerung auf die Datenassimilation, also das Annähern eines Modells an observierte Parameter durch Steuerungsparameter, überträgt.
...
% J(x_0) = \frac{1}{2}\int_0^T H(x(t),t)dt \text{ mit }H(x(t),t) \coloneqq  \|C\cdot x(V)-\xobs\|^2
\end{equation}
% mit $H(x(t),t) \coloneqq  \|C\cdot x(V)-\xobs\|^2$
...
Man beachte, dass das Zielfunktional nur abhängig vom Anfangswert $x_0$ ist. Das Ziel besteht darin, $x_0 = x_0^*$ zu finden, sodass das Optimierungsproblem
\begin{equation}
...
Für die folgenden Betrachtungen sind Hilberträume von besonderer Wichtigkeit. 
% Sämtliche Lösungstheorie sowohl bei gewöhnlichen als auch bei partiellen Differentialgleichungen basieren auf den speziellen Eigenschaften dieser Räume.
...
\begin{definition}[Adjungierte Operatoren]\label{def:adjointOperators}
...
\]
mit einer von $x$ und $d$ unabhängigen Konstanten $c>0$ gilt, $x\in N(J,J(x^{(0)}))$ und $d$ eine Abstiegsrichtung.
\end{definition}
...
Falls $x\in N(J,J(x^{(0)}))$ erfüllt ist, heißt die Richtung $d\in \R^n$ \textit{gradientenbezogen in $x$}, wenn
 \[
